...
class Agent:
    def __init__(self, args):
        ...
        if self.parameter_sharing:
            ...
            self.actor = Actor(self.args, (1, args.obs_dim1, args.obs_dim2)).to(self.device)
            self.optimizer_actor = optim.Adam(self.actor.parameters(), lr=args.actor_lr)
            print(self.actor)
        else:
            self.Replay_buffer = ReplayBuffer(self.args)
            self.actor = [Actor(self.args, (1, args.obs_matrix_number, args.obs_dim1, args.obs_dim2)).to(self.device) for _ in range(self.agent_number)]
