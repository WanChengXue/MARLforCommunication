...
class fetcher:
    ...
    def construct_latest_model_path(self):
        ...
        if self.parameter_sharing:
            self.model_path['policy_path'] = os.path.join(os.getcwd(), "{}.model".format('policy_' + self.policy_name))
        else:
            self.model_path['policy_path'] = dict()
            for index in range(self.config_dict['env']['agent_nums']):
                agent_key = 'agent_' + str(index)
                self.model_path['policy_path'][agent_key] = os.path.join(os.getcwd(), "{}.model".format('policy_' + agent_key + '_' + self.policy_name))
        self.model_path['critic_path'] = os.path.join(os.getcwd(), "{}.model".format('critic_' + self.policy_name))
    ...
    def step(self):
        ...
        if model_info is not None:
            ...
            if self.last_model_timestamp is not None:
                self.statistic.append("sampler/model_update_interval/{}".format(self.policy_name), model_info['time'] - self.last_model_timestamp)
    ...
    def _get_model(self):
        ...
        # model_info: {'policy_name': self.policy_name, 'url': url_path, 'time':timestamp}，这个timestamp是configserver收到learner发送的新模型信息时自己添加的
        # --------------- url_path是一个字典类型数据 ---------------
    ...
    def copy_model(self, url):
        critic_path = '/home/miao/Desktop/distributed_swap/' + '/'.join(url['critic_url'].split("/")[3:])
        copy_command = 'cp {} {}'.format(critic_path, self.model_path['critic_path'])
        os.system(copy_command)
        if self.parameter_sharing:
            policy_path = '/home/miao/Desktop/distributed_swap/' + '/'.join(url['critic_url'].split("/")[3:])
