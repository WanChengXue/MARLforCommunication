...
class Agent:
    ...
    def __init__(self, config_dict, statistic, agent_fetcher):
    ...
    def compute(self, agent_obs):
        ...
        with torch.no_grad():
            ...
            log_joint_prob, scheduling_action = self.net_work(conver_data_format_to_torch_interference(agent_obs), inference_mode = True)
...
class AgentManager:
    ...
    def compute(self, obs):
        ...
        joint_log_prob_list = []
        actions = []
        for agent_key in torch_format_data['obs'].keys():
            active_agent_obs = torch_format_data['obs'][agent_key]
            if self.parameter_sharing:
        ...
        return joint_log_prob_list, actions, [state_value_PF.numpy(), state_value_Edge.numpu()]
    ...
    def normalize_state_value(self, input_matrix):
        self.global_critic.popart_head_PF.normalize(input_matrix[:,0])
        self.global_critic.popart_head_Edge.normalize(input_matrix[:,1])

    def denormalize_state_value(self, input_matrix):
        # ------------ 传入的数据input_matrix是一个长度为2的列表，其中每一个元素的维度都是1*1
        self.global_critic.popart_head_PF.denormalize(input_matrix[0])
        self.global_critic.popart_head_Edge.denormalize(input_matrix[1])