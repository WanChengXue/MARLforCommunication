...
class pointer_network(nn.Module):
    ...
    def forward(self,feature_map, action=None):
        ...
        for i in range(self.max_decoder_time):
            ...
            prob_matrix = torch.softmax(weight_output, -1) # batch_size * seq_len
...
class model(nn.Module):
    def __init__(self, policy_config):
        ...
        self.backbone_linear_layer = nn.Linear(4*self.hidden_dim, self.hidden_dim)
    ...
    def forward(self, src, action_list=None, inference_mode=True):
        ...
        backbone = torch.cat([main_head_real_part_affine, main_head_img_part_affine, sum_interference_head_real_part, sum_interference_head_img_part], -1)
...
class critic(nn.Module):
    ...
    def __init__(self, policy_config):
        ...
        self.affine_layer = nn.Linear(2*self.state_feature_dim, self.hidden_dim)
    ...
    def forward(self, src):
        # 这个scr表示的是全局信息
        global_channel_matrix_real_part = src['real_part']
        global_channel_matrix_img_part = src['img_part']
        conv_real_part_matrix = torch.tanh(self.real_channel_conv_layer(1e7*global_channel_matrix_real_part)).squeeze(1)
        conv_img_part_matrix = torch.tanh(self.img_channel_conv_layer(1e7*global_channel_matrix_img_part)).squeeze(1)
        # ---------- 两个矩阵进行concatenate，得到一个 bs*20*32的矩阵 ---------------
        concatenate_matrix = torch.cat([conv_real_part_matrix, conv_img_part_matrix], -1)
...
# test_matrix['channel_matrix']['main_matrix']['real_part'] = torch.rand(2,20,16).to(0)
# test_matrix['channel_matrix']['main_matrix']['img_part'] = torch.rand(2,20,16).to(0)
# test_matrix['channel_matrix']['interference_matrix'] = dict()
# test_matrix['channel_matrix']['interference_matrix']['interfence_1'] = dict()
# test_matrix['channel_matrix']['interference_matrix']['interfence_1']['real_part'] = torch.rand(2,20,16).to(0)
# test_matrix['channel_matrix']['interference_matrix']['interfence_1']['img_part'] = torch.rand(2,20,16).to(0)
# test_matrix['channel_matrix']['interference_matrix']['interfence_2'] = dict()
# test_matrix['channel_matrix']['interference_matrix']['interfence_2']['real_part'] = torch.rand(2,20,16).to(0)
# test_matrix['channel_matrix']['interference_matrix']['interfence_2']['img_part'] = torch.rand(2,20,16).to(0)
...
# --------------- 测试critic网络 -----------------
# test_config = {}
# test_config['popart_start'] = False
# test_config['multi_objective_start'] = False
# test_config['state_feature_dim'] = 16
# test_config['hidden_dim'] = 32
# test_config['conv_channel'] = 9
# test_critic = critic(test_config)
# test_matrix = dict()
# test_matrix['real_part'] = torch.rand(2,9,20,16)
# test_matrix['img_part'] = torch.rand(2,9,20,16)
