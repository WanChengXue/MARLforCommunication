...
class model(nn.Module):
    ...
    def forward(self, src, action_list=None, inference_mode=True):
        ...
        average_user_se = src['average_user_se'] # 3*20 * 1
        user_scheduling_counts = src['user_scheduling_counts'] # 3*20*1
        ...
        main_head_average_se_affine = torch.relu(self.main_head_average_se_affine_layer(average_user_se))
        main_head_scheduling_counts_se_affine = torch.relu(self.main_head_scheduling_counts_affine_layer(user_scheduling_counts))
        # ----------- main head 和 sum_interference 进行拼接 bs * 20 * (hidden*4) ——----------------------
        backbone = torch.cat([main_head_real_part_affine, main_head_img_part_affine, main_head_average_se_affine, main_head_scheduling_counts_se_affine], -1)
...
class critic(nn.Module):
    ...
    def __init__(self, policy_config):
        ...
        # ------------------- 定义两个线性层，分别用来处理每一个用户被调度了的次数，以及每一个用户的平均信道容量 ------
        self.sheduling_counts_affine_layer = nn.Linear(1, self.hidden_dim)
        self.average_se_affine_layer = nn.Linear(1, self.hidden_dim)
        self.affine_layer = nn.Linear(4*self.hidden_dim, self.hidden_dim)
    ...
    def forward(self, src):
        ...
        average_user_se = src['average_user_se']
        user_scheduling_counts = src['user_scheduling_counts']
        ...
        affine_user_se_part = torch.relu(self.average_se_affine_layer(average_user_se))
        affine_user_scheduling_part = torch.relu(self.sheduling_counts_affine_layer(user_scheduling_counts))
        # ---------- 两个矩阵进行concatenate，得到一个 bs*20*(2*hidden_size)的矩阵 ---------------
        concatenate_matrix = torch.cat([affine_real_part, affine_img_part, affine_user_se_part, affine_user_scheduling_part], -1)
