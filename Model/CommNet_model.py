...
class Actor(nn.Module):
    ...
    def forward(self, channel_matrix):
        ...
        return Encoder_linear_data.reshape(batch_size,1,-1)
...
class Pointer_Actor(nn.Module):
    def __init__(self, args):
        ...
        self.Encoder = nn.GRU(args.rnn_input_dim, args.hidden_dim, batch_first=True)
        self.Decoder = nn.GRU(args.rnn_input_dim, args.hidden_dim, batch_first=True)
        self.Encoder_init_input = Parameter(torch.randn(1,1,args.hidden_dim))
        # define key matrix W_k and query matrix W_q
        self.W_k = Parameter(torch.randn(1, args.weight_dim, args.hidden_dim))
        self.W_q = Parameter(torch.randn(1, args.weight_dim, args.hidden_dim))
...
class CommNet_Actor(nn.Module):
    def __init__(self, args, input_shape):
        ...
        self.agent_numbers = args.n_agents
        self.communication_turns = args.communication_turns
        self.device = "cuda" if self.args.cuda else "cpu"
        if self.parameter_sharing:
            ...
            self.precoding_agent = nn.ModuleList([Actor(args, input_shape) for _ in range(self.agent_numbers)])
            self.f_comm = nn.ModuleList([nn.GRUCell(args.rnn_hidden, args.rnn_hidden) for _ in range(self.agent_numbers)])
    ...
    def forward(self, channel_matrix_list):
        batch_size = channel_matrix_list[0].shape[0]
        # 首先计算h,c，其中h和c的size都是batch_size * agent_number * feature_dim
        for turns in range(self.communication_turns):
            if turns == 0:
                if self.parameter_sharing:
                    h_list = torch.cat([self.precoding_agent(channel_matrix_list[agent_index]) for agent_index in range(self.agent_numbers)], 1).reshape(-1, self.args.rnn_hidden)
                    c_list = torch.zeros_like(h_list)
                else:
                    h_list = torch.cat([self.precoding_agent[agent_index](channel_matrix_list[agent_index]) for agent_index in range(self.agent_numbers)], 1).reshape(-1, self.args.rnn_hidden)
                    c_list = torch.zeros_like(h_list)
            else:
                # 需要把c作为GRU的隐藏单元
                h_list = self.f_comm(h_list,c_list).reshape(batch_size, self.agent_numbers, self.args.rnn_hidden)
                sum_list = torch.sum(h_list, 1).unsqueeze(1).expand_as(h_list)
                c_list = (sum_list - h_list)/(self.agent_numbers-1)
                # 最后将h,c变成二维的，进行第二次通信
                h_list = h_list.reshape(-1, self.args.rnn_hidden)
                c_list = c_list.reshape(-1, self.args.rnn_hidden)
        # 升维，然后重新reshape
        Pointer_network_input = torch.tanh(self.decoding_layer(h_list)).reshape(batch_size, self.agent_numbers, self.seq_len, -1)
        # 通信完成之后，使用Pointer网络进行决策
        scheduling_prob_list = []
        mask_list = []
        if self.parameter_sharing:
            for agent_index in range(self.agent_numbers):
                _, scheduling_prob, selected_mask, mask = self.pointer_agent(Pointer_network_input[:,agent_index,:,:])
            ...
            pass
        return torch.cat(scheduling_prob_list,0), mask_list
