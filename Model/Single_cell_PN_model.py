    ...
    def forward(self,feature_map, action=None, inference_mode=True):
        ...
        for i in range(self.max_decoder_time):
            ...
            if action is not None:
                ...
                prob_matrix_list.append(weight_output)
                terminal_batch = deepcopy(mask[:,0] == 0)
                mask_list.append(terminal_batch)
            else:
                ...
                prob_matrix_list.append(index_probs)
                # --------- 如果说当前上一个时刻的indices是0，则表示已经结束调度了，这个时刻的indices就变成0 ---------
                indices[mask[:,0] == 0] = 0
            # ------------ 这个_terminate_vector是表示那些已经结束了的batch --------
            ...
            # -------- 将已经出现过的用户进行mask操作, 已经出现过的用户的mask变成0 --------------
        ...
        if action is None:
            return torch.stack(prob_matrix_list, 1), torch.cat(scheduling_user,-1)
        else:
            # ---- prob_matrix中的每一个元素都是bs * 20，stack出来得到(bs *() seq_len +1))* 20 ------
            return torch.stack(prob_matrix_list, 1), torch.stack(mask_list, 1)
