...
from Env.Instant_Reward import Multi_cell_instant_reward, Single_cell_instant_reward
import copy
...
class Greedy:
    def __init__(self, env_dict):
        ...
        self.multi_cell_reward_calculator = Multi_cell_instant_reward(self.transmit_power, self.noise_power, self.cyclic_index_matrix, self.sector_nums, self.user_nums, self.bs_antenna_nums)
    ...
    def construct_save_path(self):
        # ----------- 构建结果保存路径 -------------
        related_path = 'data_part/Greedy_result/single_cell_greedy/' + str(self.user_nums) +'_user/'+str(self.velocity)+'KM'
    ...
    def recyle_add_user(self):
        ...
        concatenate_se = self.multi_cell_reward_calculator.calculate_instant_reward(self.channel_matrix, current_scheduling_sequence)
        return current_scheduling_sequence, concatenate_se
