...
import time
data_path = pathlib.Path("../data_part/preprocess_data/20_user/30KM/training_data_10_10.npy")
# 读取数据
preprocess_data = np.load(data_path)
# channel_matrix, user_scheduling_matrix, legal_range, noise_power, sector_power
channel_matrix = preprocess_data[0,:,0,:,0]
sector_power = [0.25]   
legal_range = [3, 16]
user_sheduling_matrix = np.random.choice(2,(1,20))
...
calculate_instant_reward(channel_matrix, user_sheduling_matrix, legal_range, noise_spectrum_density, sector_power)
end_time = time.time()
