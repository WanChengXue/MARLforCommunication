...
class Environment:
    ...
    def Calculate_init_average_user_sum_rate(self):
        ...
        for cell_index in range(self.agent_nums):
            ...
            user_channel = self.delay_TTI_data[cell_index][:,cell_id, sector_id, :]
            # 遍历所有的用户即可
    ...
    def Read_TTI_data(self):
        ...
        for agent in range(self.agent_nums):
            ...
            self.delay_source_channel.append(activate_channel_delay)
            self.TTI_data.append(activate_channel[:,:,:,0:self.bs_antenna_number]+ 1j*activate_channel[:,:,:,self.bs_antenna_number:])
            self.delay_TTI_data.append(activate_channel_delay[:,:,:,0:self.bs_antenna_number]+1j*activate_channel_delay[:,:,:,self.bs_antenna_number:])
        self.TTI_count += 1
    ...
    def Calculate_user_sum_rate(self, action):
        ...
        if self.is_reasonable:
            for cell_index in range(self.agent_nums):
                ...
                cell_select_data = self.delay_TTI_data[cell_index]
