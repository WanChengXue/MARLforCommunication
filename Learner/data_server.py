...
class data_server(basic_server):
    def __init__(self, args):
        ...
        self.traing_set = getattr(data_utils, self.config_dict['env']['replay_buffer_name'])(self.policy_config['buffer'])
        #-------------------------- 定义一些相关数据指标 ------------------------------
    ...
    def receive_data(self, socks):
        ...
        if time.time() > self.next_print_log_time:
            ...
            self.send_log({'data_server/dataserver_parse_data_time_per_min/{}'.format(self.policy_name): sum(self.parse_data_time_list)})
            self.parse_data_time_list = []
            self.socket_time_list = []
            self.recv_training_instance_count = 0
